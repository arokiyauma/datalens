...
body{{font-family:Segoe UI,Inter,Arial,sans-serif;background:radial-gradient(circle at 8% 0%,rgba(37,99,235,.10),transparent 25%),radial-gradient(circle at 92% 4%,rgba(6,182,212,.10),transparent 22%),#f4f7fb;color:#0f172a;overflow:hidden}}
.page{{width:100%;height:100vh;padding:12px 14px;display:grid;grid-template-rows:auto auto auto 1fr auto;gap:8px;overflow:hidden}}
...
.charts{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));grid-template-rows:repeat(2,minmax(0,1fr));gap:8px;min-height:0;overflow:hidden}}
.card{{min-width:0;min-height:0;background:#fff;border:1px solid #dbe4f0;border-radius:15px;padding:8px 9px;box-shadow:0 6px 16px rgba(15,23,42,.045);display:grid;grid-template-rows:auto 1fr;overflow:hidden}}
...
@media(max-width:1050px){{.kpis{{grid-template-columns:repeat(4,minmax(0,1fr))}}.charts{{grid-template-columns:repeat(2,minmax(0,1fr));grid-template-rows:repeat(4,minmax(0,1fr))}}.insight-list{{grid-template-columns:1fr 1fr}}}}
@media(max-width:700px){{body{{overflow:auto}}.page{{height:auto;min-height:100vh;overflow:visible}}.kpis{{grid-template-columns:repeat(2,minmax(0,1fr))}}.charts{{grid-template-columns:1fr;grid-template-rows:none;overflow:visible}}.card{{min-height:220px}}.insights{{align-items:flex-start}}.insight-list{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div class="page">
  ...
  <div class="footer">Generated by DataLens • One-screen dashboard view</div>
</div>
...
function draw(el,c){{
  ...
  if(type==='bar') trace=[{{x:c.labels||[],y:c.values||[],type:'bar',marker:{{color:'#2563eb'}}}}];
  else if(type==='line') trace=[{{x:c.x||[],y:c.y||[],type:'scatter',mode:'lines+markers',line:{{width:2,color:'#2563eb',shape:'spline'}},marker:{{color:'#06b6d4',size:4}}}}];
  else if(type==='area') trace=[{{x:c.x||[],y:c.y||[],type:'scatter',mode:'lines',fill:'tozeroy',line:{{width:2,color:'#2563eb'}},fillcolor:'rgba(37,99,235,.18)'}}];
  else if(type==='pie' || type==='donut') trace=[{{labels:c.labels||[],values:c.values||[],type:'pie',hole:type==='donut'?.55:0,textinfo:'label+percent',textposition:'inside'}}];
  else if(type==='histogram') trace=[{{x:c.values||[],type:'histogram',marker:{{color:'#06b6d4'}}}}];
  else if(type==='box') trace=[{{x:c.x||[],y:(c.y||[]).map(Number),type:'box',boxpoints:'outliers',jitter:.3,pointpos:0,marker:{{size:3,color:'#2563eb'}},line:{{color:'#2563eb'}}}}];
  else if(type==='scatter') trace=[{{x:c.x||[],y:(c.y||[]).map(Number),type:'scatter',mode:'markers',marker:{{size:6,color:'#0284c7',opacity:.7}}}}];
  else if(type==='heatmap') trace=[{{x:c.x||[],y:c.y||[],z:c.z||[],type:'heatmap',colorscale:'Blues',zmin:-1,zmax:1,colorbar:{{title:'Correlation',thickness:8}}}}];
  else if(type==='treemap') trace=[{{labels:c.labels||[],parents:c.parents||[],values:c.values||[],type:'treemap'}}];
  else if(type==='funnel') trace=[{{y:c.labels||[],x:c.values||[],type:'funnel'}}];
  else trace=[{{x:c.labels||c.x||[],y:c.values||c.y||[],type:'bar',marker:{{color:'#2563eb'}}}}];
...
(DATA.charts||[]).forEach((c,i)=>{{
  ...
  cg.appendChild(card);
  draw(document.getElementById('chart'+i),c);
}});
