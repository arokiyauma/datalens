...
def generate_charts(df):
    ...
    if date_columns and numeric_columns:
        ...
        if len(temp) > 0:
            ...
            charts.append({
                ...
                "x": [
                    str(x)
                    for x in grouped[date_col]
                ],
                "y": [
                    clean_value(x)
                    for x in grouped[value_col]
                ]
            })
    ...
    for column in categorical_columns:
        ...
        # Don't create charts with hundreds of categories
    ...
    if categorical_columns and numeric_columns:
        ...
        if len(temp) > 0:
            ...
            charts.append({
                "type": "bar",
                "title":
                    f"Average {numeric_col} by {category_col}",
                "column": category_col,
                "labels": [
                    str(x)
    ...
    if len(numeric_columns) >= 2:
        ...
        temp[first] = pd.to_numeric(
            temp[first],
            errors="coerce"
        )

        temp[second] = pd.to_numeric(
            temp[second],
            errors="coerce"
        )
        ...
        if len(temp) > 0:
            ...
            charts.append({
                ...
                "y": [
                    clean_value(x)
                    for x in temp[second]
                ]
            })

    # =====================================================
    # 6. SECOND NUMERIC RELATIONSHIP
    # =====================================================

    if len(numeric_columns) >= 3:

        first = numeric_columns[0]
        third = numeric_columns[2]
